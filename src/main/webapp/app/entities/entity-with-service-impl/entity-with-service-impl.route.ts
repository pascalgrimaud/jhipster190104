...
export class EntityWithServiceImplResolve implements Resolve<IEntityWithServiceImpl> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceImpl> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceImpl>) => response.ok),
                map((entityWithServiceImpl: HttpResponse<EntityWithServiceImpl>) => entityWithServiceImpl.body)
            );
        }
