...
export class EntityWithServiceClassAndDTOResolve implements Resolve<IEntityWithServiceClassAndDTO> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceClassAndDTO> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceClassAndDTO>) => response.ok),
                map((entityWithServiceClassAndDTO: HttpResponse<EntityWithServiceClassAndDTO>) => entityWithServiceClassAndDTO.body)
            );
        }
