...
@Component({
    selector: 'my-prefix-field-test-service-class-entity',
...
export class FieldTestServiceClassEntityComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.fieldTestServiceClassEntityService
            .query()
            .pipe(
                filter((res: HttpResponse<IFieldTestServiceClassEntity[]>) => res.ok),
                map((res: HttpResponse<IFieldTestServiceClassEntity[]>) => res.body)
            )
            .subscribe(
                (res: IFieldTestServiceClassEntity[]) => {
                    this.fieldTestServiceClassEntities = res;
                },
