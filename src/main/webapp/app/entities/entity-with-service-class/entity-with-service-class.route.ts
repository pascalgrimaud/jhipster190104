...
export class EntityWithServiceClassResolve implements Resolve<IEntityWithServiceClass> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceClass> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceClass>) => response.ok),
                map((entityWithServiceClass: HttpResponse<EntityWithServiceClass>) => entityWithServiceClass.body)
            );
        }
