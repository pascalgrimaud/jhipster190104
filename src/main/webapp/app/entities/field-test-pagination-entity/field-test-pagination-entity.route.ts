...
export class FieldTestPaginationEntityResolve implements Resolve<IFieldTestPaginationEntity> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IFieldTestPaginationEntity> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<FieldTestPaginationEntity>) => response.ok),
                map((fieldTestPaginationEntity: HttpResponse<FieldTestPaginationEntity>) => fieldTestPaginationEntity.body)
            );
        }
