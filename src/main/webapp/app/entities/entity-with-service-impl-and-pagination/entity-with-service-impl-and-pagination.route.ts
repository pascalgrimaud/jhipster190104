...
export class EntityWithServiceImplAndPaginationResolve implements Resolve<IEntityWithServiceImplAndPagination> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceImplAndPagination> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceImplAndPagination>) => response.ok),
                map(
