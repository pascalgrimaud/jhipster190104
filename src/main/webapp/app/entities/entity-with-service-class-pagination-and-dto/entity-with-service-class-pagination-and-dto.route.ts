...
export class EntityWithServiceClassPaginationAndDTOResolve implements Resolve<IEntityWithServiceClassPaginationAndDTO> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceClassPaginationAndDTO> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceClassPaginationAndDTO>) => response.ok),
                map(
