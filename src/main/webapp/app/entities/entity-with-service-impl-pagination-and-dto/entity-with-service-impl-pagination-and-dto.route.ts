...
export class EntityWithServiceImplPaginationAndDTOResolve implements Resolve<IEntityWithServiceImplPaginationAndDTO> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceImplPaginationAndDTO> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceImplPaginationAndDTO>) => response.ok),
                map(
