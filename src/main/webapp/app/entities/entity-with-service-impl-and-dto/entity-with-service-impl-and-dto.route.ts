...
export class EntityWithServiceImplAndDTOResolve implements Resolve<IEntityWithServiceImplAndDTO> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IEntityWithServiceImplAndDTO> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EntityWithServiceImplAndDTO>) => response.ok),
                map((entityWithServiceImplAndDTO: HttpResponse<EntityWithServiceImplAndDTO>) => entityWithServiceImplAndDTO.body)
            );
        }
