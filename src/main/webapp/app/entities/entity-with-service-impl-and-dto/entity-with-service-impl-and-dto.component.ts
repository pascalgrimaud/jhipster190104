...
@Component({
    selector: 'my-prefix-entity-with-service-impl-and-dto',
...
export class EntityWithServiceImplAndDTOComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.entityWithServiceImplAndDTOService
            .query()
            .pipe(
                filter((res: HttpResponse<IEntityWithServiceImplAndDTO[]>) => res.ok),
                map((res: HttpResponse<IEntityWithServiceImplAndDTO[]>) => res.body)
            )
            .subscribe(
                (res: IEntityWithServiceImplAndDTO[]) => {
                    this.entityWithServiceImplAndDTOS = res;
                },
