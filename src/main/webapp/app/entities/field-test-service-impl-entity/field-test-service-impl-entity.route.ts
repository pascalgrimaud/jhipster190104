...
export class FieldTestServiceImplEntityResolve implements Resolve<IFieldTestServiceImplEntity> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IFieldTestServiceImplEntity> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<FieldTestServiceImplEntity>) => response.ok),
                map((fieldTestServiceImplEntity: HttpResponse<FieldTestServiceImplEntity>) => fieldTestServiceImplEntity.body)
            );
        }
