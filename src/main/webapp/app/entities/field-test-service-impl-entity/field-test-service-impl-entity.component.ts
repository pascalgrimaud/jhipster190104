...
@Component({
    selector: 'my-prefix-field-test-service-impl-entity',
...
export class FieldTestServiceImplEntityComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.fieldTestServiceImplEntityService
            .query()
            .pipe(
                filter((res: HttpResponse<IFieldTestServiceImplEntity[]>) => res.ok),
                map((res: HttpResponse<IFieldTestServiceImplEntity[]>) => res.body)
            )
            .subscribe(
                (res: IFieldTestServiceImplEntity[]) => {
                    this.fieldTestServiceImplEntities = res;
                },
